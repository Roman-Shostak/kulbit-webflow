...
Підставляєш короткий хеш свіжого коміту (напр. `@e96c976`), тестуєш одразу, потім повертаєш `@main` коли кеш розсмокчеться. **Останній запушений коміт: `e96c976`** (десктоп-анімація is-our-clients, ADR-013). _Перед ним: `66700fe` (desktop drag-fix), `24725c9` (кольори hero-кнопки)._
...
**Інверсія напрямку на тачі (фікс 21.05.2026):** на реальному тач-екрані свайп знизу-вгору має гортати **далі** (як нативний скрол), але GSAP Observer шле для тача `onUp` саме на свайп угору → виходив зворотний напрямок. Тому в `setupObserver` визначаємо `touchInvert = matchMedia('(pointer: coarse)').matches` і для тач-девайсів міняємо знак: `onDown → -1`, `onUp → +1`. Десктоп (колесо, `pointer: fine`) — без змін. (Гібрид тач+миша з primary `pointer:fine` не інвертується — прийнятний компроміс для B2B-аудиторії: телефон/планшет = coarse, десктоп = fine.)

**Тип вводу за девайсом (фікс 22.05.2026, `66700fe`):** Observer `type` теж залежить від `pointer:coarse`: **десктоп — лише `'wheel'`** (колесо/тачпад), **тач — `'wheel,touch,pointer'`** (свайпи). Прибрали `'pointer'` на десктопі, бо він ловив **затиснуту ЛКМ + рух мишею** і трактував як скрол (небажано). На тачі `pointer`/`touch` потрібні для свайпів.
...
**Стан деплою:** усе **ЗАПУШЕНО на `origin/main`, останній коміт `e96c976`** (десктоп-анімація is-our-clients, ADR-013). Бандл ~57 KB.
