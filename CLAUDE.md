...
Підставляєш короткий хеш свіжого коміту (напр. `@1f8579b`), тестуєш одразу, потім повертаєш `@main` коли кеш розсмокчеться. **Останній запушений коміт: `1f8579b`** (мобільний hero ≤479, ADR-011).
...
### 🔖 Точка відновлення (сесія 21.05.2026 — hero + відео + стекінг + таблет + мобілка)
...
**Стан деплою:** усе **ЗАПУШЕНО на `origin/main`, останній коміт `1f8579b`** (мобільний hero ≤479, ADR-011). Бандл ~43.3 KB.

**Далі за планом:** **Крок 8 — залишок** (попап «поверни екран» для landscape 480-767 / по орієнтації + динамічний `gsap.matchMedia` замість одноразового брейкпоінта — зараз зміна ширини потребує reload), **Крок 7** (header standalone — зараз header рухається лише в hero-таймлайні) або **Крок 6** (анімації решти секцій, коли зʼявиться контент).
...
_Останнє оновлення цього файлу: 21 травня 2026 — **мобільний hero (≤479)**: переюзає таблет-хореографію `buildTabletHero` (та сама анімація, ті самі `-tablet` атрибути; одна гілка `≤479` в `registerAnimations`). Оновлено **ADR-011** (тепер таблет+мобілка ділять hero; межі брейкпоінтів 992/768/479; 480-767 — попап landscape), §4-таблиця, §11-план (Крок 8 частково). Раніше: **ADR-011** база (таблет-hero 3 кроки, відео→16:9 + кнопка в центр + пауза/мут відео по видимості), **ADR-009** (data-driven таймлайни), **ADR-010** (стекінг секцій), Vimeo bg-відео. **Точка відновлення — в кінці §11**. Останній коміт: `1f8579b`. Наступне: залишок Кроку 8 (попап landscape + динамічний `gsap.matchMedia`) / Крок 7 (header) / Крок 6 (решта секцій)._
