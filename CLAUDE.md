...
Підставляєш короткий хеш свіжого коміту (напр. `@66700fe`), тестуєш одразу, потім повертаєш `@main` коли кеш розсмокчеться. **Останній запушений коміт: `66700fe`** (десктоп: drag мишею не скролить). _Перед ним: `24725c9` (кольори hero-кнопки), `05adb4e` (matchMedia)._
...
**Стан деплою:** усе **ЗАПУШЕНО на `origin/main`, останній коміт `66700fe`** (десктоп: drag мишею не скролить; Observer type='wheel'). Бандл ~52.7 KB.
