...
_Останнє оновлення цього файлу: 21 травня 2026 — додано **ADR-011** (брейкпоінт-залежний таблет-hero 768-991: 3 кроки, відео→16:9 + стекінг секції 2 + кнопка в центр 16:9; + пауза/мут відео по видимості, загальне). Запечено: `buildTabletHero`/`tabletHeroStep` (`03-sections.js`), `updateVideoVisibility` (`08-video.js`). Брейкпоінт визначається одноразово при init (reload для зміни). Раніше цієї сесії: **ADR-009** (data-driven таймлайни), **ADR-010** (стекінг секцій), Vimeo bg-відео, desktop+tablet hero. **Точка відновлення — в кінці §11** (там же — чек-лист Webflow, вкл. `-tablet`). Наступне: push таблета → перевірка → Крок 8 (мобілка + динамічний matchMedia) / Крок 7 (header) / Крок 6 (решта секцій)._
