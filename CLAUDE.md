...
Підставляєш короткий хеш свіжого коміту (напр. `@7078aed`), тестуєш одразу, потім повертаєш `@main` коли кеш розсмокчеться. **Останній запушений коміт: `7078aed`** (ховер-бордер: padding-box + офсет 2px). _Перед ним: `05adb4e` (matchMedia + таймінг паузи відео)._
...
**Стан деплою:** усе **ЗАПУШЕНО на `origin/main`, останній коміт `7078aed`** (ховер-бордер: padding-box + офсет 2px). Бандл ~52.2 KB.
