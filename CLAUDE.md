...
Підставляєш короткий хеш свіжого коміту (напр. `@9931dd6`), тестуєш одразу, потім повертаєш `@main` коли кеш розсмокчеться. **Актуальний бандл — на `9931dd6`** — для commit-pinned тесту коду підставляй саме `@9931dd6`. _Стабільна база: hero + is-our-clients + is-projects (усі девайси) + контролі відео + персистентність + поворот не ламає + **кнопки-автопрогравання** + **scramble за атрибутом** (`11-scramble.js`, ADR-017) + **is-our-services горизонтальний свап карток** (ADR-018) + **is-working-process картки** (ADR-019, stack з падінням, усі брейкпоінти; SVG — окремо). Усе підтверджено Romanom (desktop + iPhone до ADR-018; ADR-019 чекає тесту після пушу)._
...
- [x] **is-working-process — КАРТКИ готові** на всіх брейкпоінтах (ADR-019, `9931dd6`): stack-cards з падінням активної (desktop — горизонтальний стек + діагональ; tablet/mobile — Z-stack + пряме падіння). SVG-частина (нижня лінія + крапки + 3 стрілки) — окремою ітерацією
...
### 🔖 Точка відновлення (29.05.2026 — стабільна база + wp-картки)

**Поточний стабільний стан = `9931dd6`.** Hero + is-our-clients + is-our-services (ADR-018) + is-projects (усі девайси) + контролі відео + персистентність + стійкість до повороту + кнопки-автопрогравання + scramble за атрибутом + **is-working-process КАРТКИ (ADR-019, stack-cards з падінням активної, усі брейкпоінти)** — **усе підтверджене Romanом** до ADR-018 (desktop + iPhone Safari); **ADR-019 чекає тесту на staging** (запушено, commit-pinned `@9931dd6`). Бандл ~132 KB.
...
**Стан верстки:** 9 зупинок розмічені; hero + is-our-clients + is-projects + is-our-services + **is-working-process (картки готові, SVG в розмітці)** ✅. **Решта секцій + footer — Roman верстає.** Атрибути hero та Vimeo-embed — у `docs/hero-reference.md`. ⚠️ дефект верстки: `.hero-video-mask`/`.hero-video-wrapper` мають `overflow:visible` — варто `hidden` (cover-iframe інакше може розпирати документ на iOS).

**Далі (по пріоритету):** (1) Roman тестує is-working-process картки на staging — підтвердження/поправки. (2) SVG-частина is-working-process (нижня лінія drawSVG, стрілки dasharray, gradient-shadow opacity, dot color change — все вже з атрибутами). (3) Решта елементів секції — week-теги, labels Kulbit/Traditional, send-icon кольори. (4) Крок 6 — анімації решти секцій (чекає верстки Roman). (5) Крок 9 — попап-форма. Необов'язкове: auto-landscape fullscreen на iOS.
