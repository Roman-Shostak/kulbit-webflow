...
Підставляєш короткий хеш свіжого коміту (напр. `@cb4a5a4`), тестуєш одразу, потім повертаєш `@main` коли кеш розсмокчеться. **Останній запушений коміт: `cb4a5a4`** (напрямок скролу за типом події). _Перед ним: `e96c976` (is-our-clients десктоп ADR-013), `66700fe` (drag-fix)._
...
**Реалізація:** `buildOurClients` (`03-sections.js`, у desktop-гілці matchMedia) → `section.oc = { enter, reset, step, state }`; `advance`/`goToSection` делегують на `section.oc`; `01-init.js` реєструє `ScrambleTextPlugin`. ⚠️ **Tablet/mobile для цієї секції — TBD** (на цих брейкпоінтах `buildOurClients` НЕ викликається, тож набори 2/3 наразі накладаються — лейаут інший, робитимемо окремо). Перевірено покроково консольними тестами; запечено `e96c976`.
...
**Стан деплою:** усе **ЗАПУШЕНО на `origin/main`, останній коміт `cb4a5a4`** (напрямок скролу за типом події; is-our-clients десктоп — `e96c976`). Бандл ~57 KB.
