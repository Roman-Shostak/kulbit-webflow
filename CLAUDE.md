...
Підставляєш короткий хеш свіжого коміту (напр. `@e38f0bc`), тестуєш одразу, потім повертаєш `@main` коли кеш розсмокчеться. **Останній запушений коміт: `e38f0bc`** (ховер-бордер: масштаб 1:1 + офсет). _Перед ним: `05adb4e` (matchMedia + таймінг паузи відео)._
...
**Стан деплою:** усе **ЗАПУШЕНО на `origin/main`, останній коміт `e38f0bc`** (ховер-бордер: масштаб 1:1 + офсет). Бандл ~52.2 KB.
