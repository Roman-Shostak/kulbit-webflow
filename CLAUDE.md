...
Підставляєш короткий хеш свіжого коміту (напр. `@7b9f03a`), тестуєш одразу, потім повертаєш `@main` коли кеш розсмокчеться. **Останній запушений коміт: `7b9f03a`** (ховер-бордер: transform-safe, лінія рівно по бордеру). _Перед ним: `05adb4e` (matchMedia + таймінг паузи відео)._
...
- [x] **Ховер-ефект бордера кнопки** — **завершено** (21.05.2026, `09-button-border.js`). На елементах з атрибутом `data-kulbit-border` синій бордер (`--colors--blue`) промальовується від точки курсора в обидва боки (SVG-обведення + анімований `stroke-dasharray`, товщина = товщині бордера), при відведенні стягується до точки виходу. Матова лінія + проявлення `opacity`, лінійний easing, 0.3с. Підтримка ресайзу й кількох елементів. **Дія Romana у Webflow:** додати кнопці custom attribute `data-kulbit-border`. **Фікс вирівнювання лінії (22.05.2026, `7b9f03a`):** лінія **рівно по бордеру**. Ключове: (1) розмір беремо з `offsetWidth/offsetHeight` (ЛЕЙАУТ-розмір, **імунний до CSS-transform**), а **НЕ** `getBoundingClientRect()` — той дає ВІЗУАЛЬНИЙ (зменшений трансформом) розмір, а на кнопці/предку є transform (стекінг/анімації) → рамка виходила менша й тулилась у лівий-верхній кут; (2) SVG розтягуємо точно на border-box через `inset: -sw` з усіх боків (top/left/right/bottom) — лейаут-відносно, дробово, без округлення → права/нижня грань сходяться; (3) `viewBox=offsetWidth/offsetHeight` + `preserveAspectRatio:none` розтягується рівно під box; (4) `config.offset` дефолт **0** (штрих по бордеру; >0 виносить назовні, `overflow:visible`). ⚠️ **Урок:** для оверлеїв над трансформованими елементами — `offsetWidth`/`inset`, не `getBoundingClientRect`.
...
**Стан деплою:** усе **ЗАПУШЕНО на `origin/main`, останній коміт `7b9f03a`** (ховер-бордер: transform-safe, рівно по бордеру). Бандл ~52.2 KB.
