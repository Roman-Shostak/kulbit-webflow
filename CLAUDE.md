...
### 🔖 Точка відновлення (29.05.2026 — стабільна база + wp SVG-reveal)

**Поточний стабільний стан = `0d35cbf`.** Hero + is-our-clients + is-our-services (ADR-018) + is-projects (усі девайси) + контролі відео + персистентність + стійкість до повороту + кнопки-автопрогравання + scramble за атрибутом + **is-working-process ПОВНІСТЮ (ADR-019 + SVG-ітерація: картки stack-падіння + SVG-reveal появи + крок-синхрон стрілок/крапок + швидкий реверс на вихід; desktop/tablet повна, mobile спрощено)** — **усе підтверджене Romanом** (desktop + iPhone Safari). Бандл ~145 KB. Commit-pinned тест: `@0d35cbf`.
