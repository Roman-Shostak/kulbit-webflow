...
Підставляєш короткий хеш свіжого коміту (напр. `@b4a975d`), тестуєш одразу, потім повертаєш `@main` коли кеш розсмокчеться. **Останній запушений коміт: `b4a975d`** (ховер-лінія під outline-кнопку). _Перед ним: `05adb4e` (matchMedia + таймінг паузи відео)._
...
- [x] **Ховер-ефект лінії кнопки** — **завершено** (`09-button-border.js`). На елементах `data-kulbit-border` синя лінія (`--colors--blue`) промальовується від точки курсора в обидва боки (SVG-обведення + анімований `stroke-dasharray`, товщина = `outline-width`), при відведенні стягується до точки виходу. Матова лінія + `opacity`, лінійний easing, 0.3с. Ресайз + кілька елементів.
...
**Стан деплою:** усе **ЗАПУШЕНО на `origin/main`, останній коміт `b4a975d`** (ховер-лінія: SVG 100% під outline-кнопку). Бандл ~52.2 KB.
