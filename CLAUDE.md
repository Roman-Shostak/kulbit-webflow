...
- [x] **Ховер/фокус-ефект лінії кнопки** — **завершено** (`09-button-border.js`). На елементах `data-kulbit-border` синя лінія (`--colors--blue`), SVG-обведення + анімований `stroke-dasharray`, товщина = `outline-width`, лінійний easing 0.3с. **Два режими (`state.mode`):** **ховер** — промальовка від точки курсора в обидва боки (при відведенні стягується до точки виходу); **фокус** (`focusin`/`focusout`, клавіатура/клік) — повне коло плавно проявляється `opacity 0→1` (`drawFull`). Лінія активна поки є ховер АБО фокус (прапорці `hovered`/`focused`); при `p=1` обидва режими дають однакове повне коло → перемикання безшовне. Ресайз + кілька елементів.
  - **Кольори hero-кнопки (`.button.is-hero` лише):** на активному стані (ховер/фокус) додатково — текст → `var(--colors--white-10)`, `currentColor` у SVG-іконках → `var(--colors--blue)` (наш бордер-SVG пропускається, бо в нього явний stroke). `setColors(hovered||focused)` синхронно зі станом лінії; інлайн-стилі знімаються при деактивації. Гейт `el.matches('.button.is-hero')`.
