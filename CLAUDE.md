...
### 🔖 Точка відновлення (30.05.2026 — wp SVG-reveal готова + прод-білд)

**Поточний стан = `0b77b01` (HEAD).** Код секцій (вкл. WP SVG-reveal) — на `0d35cbf`; `7af5897` додав прод-зачистку білда; `0b77b01` — доку. Hero + is-our-clients + is-our-services (ADR-018) + is-projects (усі девайси) + контролі відео + персистентність + стійкість до повороту + кнопки-автопрогравання + scramble за атрибутом + **is-working-process ПОВНІСТЮ (ADR-019 + SVG-ітерація: картки stack-падіння + SVG-reveal появи + крок-синхрон стрілок/крапок + швидкий реверс на вихід; desktop/tablet повна, mobile спрощено)** — **усе підтверджене Romanом** (desktop + iPhone Safari). **Білд: `node build.js` = DEV (логи+коментарі), `node build.js --prod` = чистий прод (78 KB зі 145, 0 console).** Commit-pinned: `@7af5897` (прод-чистий) / `@0d35cbf` (з логами).
...
**Стан верстки:** 9 зупинок розмічені; hero + is-our-clients + is-projects + is-our-services + **is-working-process (картки+SVG готові)** ✅. **Решта секцій + footer — Roman верстає.** Атрибути hero та Vimeo-embed — у `docs/hero-reference.md`. ⚠️ дефект верстки: `.hero-video-mask`/`.hero-video-wrapper` мають `overflow:visible` — варто `hidden` (cover-iframe інакше може розпирати документ на iOS).
