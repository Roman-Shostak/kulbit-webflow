...
Підставляєш короткий хеш свіжого коміту (напр. `@15422ae`), тестуєш одразу, потім повертаєш `@main` коли кеш розсмокчеться. **Останній запушений коміт: `15422ae`** (is-our-clients: скрамбл заголовків керується видимістю IN/OUT). _Перед ним: `b022ae2` (плавний прогрес), `d236286` (фікс реверсу таблета)._
...
**Таблет-механіка (`buildOurClients('tablet')`):** набори — вертикальні колонки (CSS ≤991: `flex-flow:column; height:51rem; overflow:hidden`). Та сама логіка по наборах + **окремий крок «зсув карток на 1»** на 6-карткових наборах (1 і 3; набір 2 має 4 картки — без зсуву). Послідовність кроків: поява(набір1) → зсув набору1 → набір2 → набір3+скрамбл → зсув набору3 → наступна секція. Реалізовано `STAGES`-машиною (`{cur, shifts[], prog}`); зсув = переклад карток усередині врапера на `cardStep` (відстань між сусідніми картками, з `getBoundingClientRect`). Прогрес — по наборах (зсуви його не рухають).
...
**Реалізація:** `buildOurClients(mode)` (`03-sections.js`, `mode` = `'desktop'`/`'tablet'`, спільні скрамбл/прогрес/текст-утиліти) → `section.oc = { prepare, enter, reset, step, state }`; `advance`/`goToSection`/`tabletHeroStep` делегують; `01-init.js` реєструє `ScrambleTextPlugin`. matchMedia: desktop-гілка → `'desktop'`, tablet-гілка (768-991) → `'tablet'`. ⚠️ **Mobile (≤479) — TBD** (`buildOurClients('mobile')` ще не викликається; набори 2/3 там накладаються — лейаут інший, робитимемо окремо). Десктоп `e96c976`, таблет `9e1a958`, фікси таймінгу/реверсу `d236286`; перевірено консольними тестами.
...
**📌 TL;DR для наступної сесії:** Hero-секція повністю завершена й підтверджена на всіх девайсах. **Кроки 1-8 + Крок 7 закриті.** Лишився **Крок 6** — анімації решти 7 секцій + footer, але вони ЗАБЛОКОВАНІ контентом: Roman зараз верстає ці секції у Webflow. Коли зʼявиться розмітка/контент — навішуємо на елементи `data-kulbit-*` атрибути (ADR-009) або reveal-кроки `[data-kulbit-step]`, рушій уже готовий. **Секція is-our-clients — десктоп + ТАБЛЕТ ✅ ГОТОВО Й ПІДТВЕРДЖЕНО Romanом (ADR-013, з усіма фіксами: реверс через OC, таймінг прогресу, скрамбл заголовків за видимістю IN/OUT), лишилась тільки МОБІЛКА цієї секції.** Усе запушено (`15422ae`), бандл ~60 KB, робоче дерево чисте.
...
**Стан деплою:** усе **ЗАПУШЕНО на `origin/main`, останній коміт `15422ae`** (is-our-clients: скрамбл заголовків IN/OUT за видимістю; таблет `9e1a958`, десктоп `e96c976`). Бандл ~60 KB.
