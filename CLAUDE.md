...
Підставляєш короткий хеш свіжого коміту (напр. `@71b75b1`), тестуєш одразу, потім повертаєш `@main` коли кеш розсмокчеться. **Останній запушений коміт: `71b75b1`** (ховер-бордер: лінія рівно по бордеру). _Перед ним: `05adb4e` (matchMedia + таймінг паузи відео)._
...
- [x] **Ховер-ефект бордера кнопки** — **завершено** (21.05.2026, `09-button-border.js`). На елементах з атрибутом `data-kulbit-border` синій бордер (`--colors--blue`) промальовується від точки курсора в обидва боки (SVG-обведення + анімований `stroke-dasharray`, товщина = товщині бордера), при відведенні стягується до точки виходу. Матова лінія + проявлення `opacity`, лінійний easing, 0.3с. Підтримка ресайзу й кількох елементів. **Дія Romana у Webflow:** додати кнопці custom attribute `data-kulbit-border`. **Фікс вирівнювання лінії (22.05.2026, `71b75b1`):** лінія тепер **рівно по бордеру**. (1) **дробові розміри** border-box через `getBoundingClientRect()`, а не округлені `offsetWidth/offsetHeight` (інакше права/нижня грань недотягували на частку пікселя — похибка округлення накопичується до дальніх граней); (2) явні px розміри SVG `== viewBox` → **масштаб 1:1 на обох осях** (без перекосу); (3) оверлей точно на border-box (`top/left:-sw`); (4) `config.offset` дефолт **0** (штрих по центру бордера = повний збіг; >0 виносить назовні симетрично, `overflow:visible`).
...
**Стан деплою:** усе **ЗАПУШЕНО на `origin/main`, останній коміт `71b75b1`** (ховер-бордер: лінія рівно по бордеру). Бандл ~52.2 KB.
