...
Підставляєш короткий хеш свіжого коміту (напр. `@b022ae2`), тестуєш одразу, потім повертаєш `@main` коли кеш розсмокчеться. **Останній запушений коміт: `b022ae2`** (is-our-clients: скрамбл коли текст видно + плавний прогрес назад). _Перед ним: `d236286` (фікс реверсу таблета), `9e1a958` (таблет-анімація)._
...
- **Скрамбл заголовків — КОЛИ ТЕКСТ СТАЄ ВИДНО** (`b022ae2`, а не на повному накритті): `IntersectionObserver` на H2/label (`threshold 0.9`) → скрамбл щойно текст у видимій зоні; фолбек у `enter()` (якщо IO не встиг). Загальне правило для всіх скрамбл-заголовків на появі (виняток — перепис параграфів на набір 3, навмисне посеред скролу). IO прибирається в `teardownHero` (`oc.dispose`).
...
**Стан деплою:** усе **ЗАПУШЕНО на `origin/main`, останній коміт `b022ae2`** (is-our-clients: IO-скрамбл + плавний прогрес; таблет `9e1a958`, десктоп `e96c976`). Бандл ~60 KB.
