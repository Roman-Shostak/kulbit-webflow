...
Підставляєш короткий хеш свіжого коміту (напр. `@463704d`), тестуєш одразу, потім повертаєш `@main` коли кеш розсмокчеться. **Останній запушений коміт: `463704d`** (фікс інверсії скролу на тачі + клас іконок).
...
**Фікси (21.05.2026, коміт `463704d`):** (1) **інверсія напрямку скролу на тачі** — на телефоні свайп знизу-вгору тепер гортає далі (`touchInvert` у `setupObserver`, лише `pointer:coarse`; десктоп без змін; деталі — ADR-008); (2) **клас іконок звуку** `.icon-24` → `.icon-24-24-16` (`08-video.js`). ⚠️ Інверсію перевірено в device-toolbar — **на реальному телефоні підтвердити після деплою**.

**Стан деплою:** усе **ЗАПУШЕНО на `origin/main`, останній коміт `463704d`**. Бандл ~47.0 KB.
...
_Останнє оновлення цього файлу: 21 травня 2026 — **попап landscape (`06-responsive.js`, ADR-004)**: детект по орієнтації (`landscape` + `max-height ≤ config.landscapeMaxHeight=500` + `pointer: coarse`), на landscape — попап + Observer off + пауза відео; розмітка `[data-kulbit-landscape-popup]`. Оновлено **ADR-004** (блок реалізації), §5-config, §11-план (Крок 8 майже завершено). Раніше цієї сесії: **мобільний hero (≤479)** переюзає `buildTabletHero` (ADR-011), **ADR-009** (data-driven таймлайни), **ADR-010** (стекінг секцій), Vimeo bg-відео, desktop+tablet hero. **Точка відновлення — в кінці §11**. Останній коміт: `463704d` (фікси: інверсія скролу на тачі + клас іконок `.icon-24-24-16`). Наступне: залишок Кроку 8 (динамічний `gsap.matchMedia`) / Крок 7 (header) / Крок 6 (решта секцій)._
