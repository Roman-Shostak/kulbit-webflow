...
### ADR-011: Брейкпоінт-залежний таблет-hero + пауза/звук відео по видимості

**Рішення:** окрема **таблет-хореографія hero** (768-991px), що сплітається зі стекінгом наступної секції; та **загальна пауза/мут відео**, коли воно накрите.

**Брейкпоінт hero** визначається ОДНОРАЗОВО при `init` (`registerAnimations`): `≥992` → desktop-таймлайни (`buildDesktopAnimations`, ADR-009), `768-991` → `buildTabletHero`, `<768` → без анімацій (Крок 8). Зміна брейкпоінта потребує reload (динамічний `gsap.matchMedia` — на Крок 8, щоб не ускладнювати зараз). Атрибути таблета — з суфіксом `-tablet` (`data-kulbit-y-tablet` тощо), живуть на тих самих елементах, що й десктопні.
...
### 🔖 Точка відновлення (сесія 21.05.2026 — hero + відео + стекінг + таблет)
