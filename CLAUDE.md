...
Підставляєш короткий хеш свіжого коміту (напр. `@9e1a958`), тестуєш одразу, потім повертаєш `@main` коли кеш розсмокчеться. **Останній запушений коміт: `9e1a958`** (таблет-анімація is-our-clients ADR-013). _Перед ним: `cb4a5a4` (напрямок скролу за подією), `e96c976` (is-our-clients десктоп)._
...
**Реалізація:** `buildOurClients(mode)` (`03-sections.js`, `mode` = `'desktop'`/`'tablet'`, спільні скрамбл/прогрес/текст-утиліти) → `section.oc = { enter, reset, step, state }`; `advance`/`goToSection` делегують; `01-init.js` реєструє `ScrambleTextPlugin`. matchMedia: desktop-гілка → `'desktop'`, tablet-гілка (768-991) → `'tablet'`. ⚠️ **Mobile (≤479) — TBD** (`buildOurClients('mobile')` ще не викликається; набори 2/3 там накладаються — лейаут інший, робитимемо окремо). Десктоп запечено `e96c976`, таблет `9e1a958`; перевірено покроково консольними тестами.
...
**Стан деплою:** усе **ЗАПУШЕНО на `origin/main`, останній коміт `9e1a958`** (таблет-анімація is-our-clients; десктоп — `e96c976`, напрямок скролу — `cb4a5a4`). Бандл ~59 KB.
