...
Підставляєш короткий хеш свіжого коміту (напр. `@d236286`), тестуєш одразу, потім повертаєш `@main` коли кеш розсмокчеться. **Останній запушений коміт: `d236286`** (is-our-clients таблет: фікс реверсу + таймінг появи). _Перед ним: `9e1a958` (таблет-анімація), `e96c976` (десктоп)._
...
**Поява/таймінг (фікс `d236286`):** прогрес-бар і скрамбл заголовків (H2/label) спрацьовують **лише на ПОВНОМУ накритті** (коли секція торкнулась верху екрана), а не під час наїзду. Механіка: `oc.prepare()` (під час наїзду — заголовки `autoAlpha:0`, прогрес 0%) → `oc.enter()` (на завершенні наїзду — прогрес 0→1/3 + скрамбл заголовків). Десктоп: `goToSection` кличе `prepare` на старті слайду, `enter` в `onComplete`. Це загальне правило для всіх скрамбл-заголовків на появі (виняток — перепис параграфів на набір 3, що навмисне посеред скролу).
...
**Стан деплою:** усе **ЗАПУШЕНО на `origin/main`, останній коміт `d236286`** (is-our-clients таблет фікси; десктоп `e96c976`, таблет `9e1a958`). Бандл ~60 KB.
