...
Підставляєш короткий хеш свіжого коміту (напр. `@3978c4d`), тестуєш одразу, потім повертаєш `@main` коли кеш розсмокчеться. **Останній запушений коміт: `3978c4d`** (ВІДКАТ до стабільної `f0928a9`: код = desktop is-projects + скрол над відео; усе експериментальне прибрано, ADR-016). _Стабільна база; tablet/mobile/персистентність/landscape — у git-історії `50fb693`..`1500d68` для майбутнього re-approach._
...
> ⛔ **ВІДКАЧЕНО 26.05.2026 (`3978c4d`).** Цей блок вніс «велику кількість проблем» (вкл. регресію скролу — див. урок у §9), тож код повернуто до стабільної `f0928a9` (desktop-only is-projects). **Опис нижче — НЕ поточний стан коду**, а специфікація для майбутнього re-approach: повертати фічі по ОДНІЙ, з тестом кожної. Усе живе в git-історії (`50fb693`..`1500d68`). Найімовірніший порядок повернення: (1) tablet/mobile свап вікном 3 (з фіксом `section.el`!), (2) персистентність, (3) landscape/fullscreen.
...
### 🔖 Точка відновлення (26.05.2026 — ВІДКАТ до стабільної бази, ADR-016 відкочено)

**📌 TL;DR (26.05.2026, після відкату `3978c4d`):** Спроба tablet/mobile свапу + персистентності + landscape (ADR-016) внесла багато проблем (вкл. регресію скролу) → **відкотили код до стабільної `f0928a9`**. **Поточний стабільний стан:** hero (всі девайси), is-our-clients (всі 3 брейкпоінти), **is-projects лише DESKTOP** (плеєр ADR-014 + свап ADR-015 + прогрес), скрол над відео. Бандл ~95 KB. Усе підтверджене, скрол працює. Експериментальні фічі — в git-історії (`50fb693`..`1500d68`), повертати по ОДНІЙ із тестом (порядок і урок — ADR-016 + §9). **Далі: з цієї бази обережно — почати з tablet/mobile свапу is-projects (з фіксом `section.el`).**


**Стан is-projects (26.05.2026, після відкату):** ✅ **DESKTOP** — плеєр (ADR-014) + свап одного відео на повну + прогрес (ADR-015). ⛔ **tablet/mobile свап ВІДКАЧЕНО** (ADR-016, нестабільність) — на менших екранах поки немає свапу (буде re-approach). Розмітка вже готова: група `[data-kulbit-project-group]` на `.projects-cards-wrapper`, 2 відео `[data-kulbit-project-video]` + END `[data-kulbit-project-end]`.
...
_Останнє оновлення цього файлу: 26 травня 2026 — **ВІДКАТ до стабільної `f0928a9`** (`3978c4d`): ADR-016 (tablet/mobile свап + персистентність + landscape + UX) внесла багато проблем (вкл. регресію скролу), тож код повернуто до стабільного стану. **Поточне:** hero + is-our-clients (всі девайси) + is-projects DESKTOP (плеєр+свап+прогрес) + скрол над відео — усе підтверджене. Бандл ~95 KB. Експериментальні фічі — в git-історії (`50fb693`..`1500d68`), повертати по одній з тестом. **Точка відновлення — в кінці §11.** Наступне: re-approach tablet/mobile свапу is-projects (з фіксом `section.el`), далі решта секцій._
