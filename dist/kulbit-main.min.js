/* Kulbit Webflow — production bundle — 2026-06-04T17:22:12.253Z */
(() => {
...
(() => {
  ...
  app.registerAnimations = () => {
    ...
    app.mm.add('(min-width: 768px) and (max-width: 991px)', () => {
      ...
      app.buildTraditional('tablet');
      app.buildFooterScroll('tablet');
      app.restoreSection();
      return () => app.teardownHero();
    });
    app.mm.add('(max-width: 479px)', () => {
      ...
      app.buildTraditional('mobile');
      app.buildFooterScroll('mobile');
      app.restoreSection();
      return () => app.teardownHero();
    });
  ...
  app.teardownHero = () => {
    ...
    app.sections.forEach((s) => {
      ...
      if (s.ft && s.ft.dispose) s.ft.dispose();
      s.ft = null;
      if (s.tp && s.tp.dispose) s.tp.dispose();
  ...
  app.buildOurClients = (mode) => {
    ...
    const morphParagraph = (p, segs) => {
      if (typeof ScrambleTextPlugin === 'undefined') { buildSegDOM(p, segs, true); return; }
      gsap.killTweensOf(p);
      gsap.to(p, {
  ...
  app.buildFooterScroll = (mode) => {
    ...
    const savedFooterH = footer.el.style.height;
    ...
    footer.ft = {
      ...
      step(dir) {
        if (maxShift <= 0) return false;
        if (dir > 0) {
          if (pos >= maxShift) return true;
      ...
      reset() { pos = 0; recompute(); gsap.set(container, { y: 0 }); },
      recompute,
      dispose() {
        if (onVV) {
  ...
  app.buildTraditional = (mode) => {
    ...
    if (!section) return;
    const el = section.el;
    const pageSize = mode === 'desktop' ? 4 : 2;
    ...
    const cardDots = [['1'], ['2'], ['3-1', '3-2'], ['4']];
    const DEMO_PCT = [40, 65, 55, 85];
    ...
    const headPara = headEl && headEl.querySelector('.text-size-production-head:not(.is-second)');
    const headSecond = headEl && headEl.querySelector('.text-size-production-head.is-second');
    ...
    const swapHead = (toBlue, tl, at) => {
      if (!headPara) return;
      const apply = () => { headPara.classList[toBlue ? 'remove' : 'add']('text-color-red'); };
      if (tl) {
    ...
    const forwardTo = (tl, s) => {
      ...
      if (crossing) {
        ...
        const endC = cascadeOut(tl, pages[pageOfState(s - 1)], at0);
        if (crossingSvg) deactivateRed(tl, at0);
        tl.call(() => setPageDisplay(pageOfState(s)), null, endC);
        if (crossingSvg && blue.center) tl.to(blue.center, { autoAlpha: 1, duration: STEP * 0.5, ease: EASE }, endC);
        if (crossingSvg) swapHead(true, tl, endC);
      }
      const g = groupOf(s), li = liOf(s);
      if (s === 4 || s === 8) addFinal(tl, g, s); else addCard(tl, g, li, s);
    };
    const backFrom = (tl, s) => {
      const g = groupOf(s), li = liOf(s);
    ...
    const showEnd = () => {
      ...
      plain.cards.forEach((c, li) => { gsap.set(c, { autoAlpha: 1, y: 0 }); if (plain.progFills[li]) gsap.set(plain.progFills[li], { width: pctOf(plain, li) + '%' }); });
      swapHead(true, null);
    ...
    const api = {
      prepare() {
        ...
        setPageDisplay(0);
        if (red.center) gsap.set(red.center, { autoAlpha: 1 });
        ...
        if (blue.center) gsap.set(blue.center, { autoAlpha: 0 });
  ...
  app.goToSection = (index, instant, dir) => {
    ...
    if (instant) {
      ...
      if (target.isFooter && target.ft) target.ft.reset();
      if (target.isTraditional && target.tp) target.tp.reset(dir < 0);
      if (app.updateVideoVisibility) app.updateVideoVisibility();
      return;
    }
    ...
    if (dir > 0) {
      ...
      if (target.isFooter && target.ft) target.ft.reset();
      if (target.isTraditional && target.tp) target.tp.prepare();
      gsap.to(target.el, {
        yPercent: 0,
        ...
        onComplete: () => {
          ...
          if (target.isTraditional && target.tp) target.tp.enter();
        }
      });
    } else {
      ...
      if (target.isTraditional && target.tp) target.tp.reset(true);
  ...
  app.advance = (dir) => {
    ...
    if (section.isFooter && section.ft) {
      if (section.ft.step(dir)) return;
      app.goToSection(app.currentSectionIndex + dir, false, dir);
      return;
    }

    if (section.isTraditional && section.tp) {
      if (section.tp.step(dir)) return;
      app.goToSection(app.currentSectionIndex + dir, false, dir);
      return;
    }
  ...
  app.autoAdvanceTo = (targetIndex) => {
    ...
    if (t === app.currentSectionIndex || app.isAnimating) return;
    const dir = t > app.currentSectionIndex ? 1 : -1;
    const saved = app.config.scrollDuration;
    app.config.scrollDuration = app.config.anchorDuration;
    app.goToSection(t, false, dir);
    app.config.scrollDuration = saved;
...
(() => {
  ...
  const setupElement = (el) => {
    ...
    const build = () => {
      ...
      rect.setAttribute('width', Math.max(0, (w - sw) + 2 * off));
